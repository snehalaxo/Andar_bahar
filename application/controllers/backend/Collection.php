...
class Collection extends MY_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['Retailer_model', 'Setting_model','Collection_model']);
    }
    
          public function index()
    {
        $batch = $this->input->post('batch_id');
        $defaultbatch= $this->Collection_model->defaultbatch();
        $batch_id= ($batch) ? ($batch) : ($defaultbatch->id) ;
        $data = [
            'title' => 'Collection',
            'retailer_list' => $this->Collection_model->Retailer_list($batch_id),
        ];

        template('Collection/list', $data);
    }
    ...
    public function retailer_collection()
    {
        ...
        $data = [
            'title' => 'Collection',
            'retailer_list' => $this->Collection_model->Retailer_collection($batch_id),
        ];
    ...
    public function edit()
    {
        $data = [
            'title' => 'Edit Retailer',
            'User' => $this->Collection_model->buy()
        ];

        template('Collection/edit', $data);
    }
