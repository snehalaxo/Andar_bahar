...
class Collection_model extends MY_Model
{
       ...
       public function Retailer_list($batch_id)
    {
   
         $Query = $this->db->query(' SELECT tbl_admin.id,tbl_admin.first_name,tbl_admin.last_name, axo_bet.*, ax_claim_ticket.* FROM tbl_admin LEFT JOIN (SELECT retailer_id, SUM(total) AS bet FROM axo_bet  where Batch_id='.$batch_id.' GROUP BY retailer_id) as axo_bet ON tbl_admin.id = axo_bet.retailer_id LEFT JOIN (SELECT retailer_id, SUM(winning_amount) AS wining FROM ax_claim_ticket  where  Batch_id='.$batch_id.' GROUP BY retailer_id) as ax_claim_ticket ON tbl_admin.id = ax_claim_ticket.retailer_id WHERE role != "admin";');
       
        return $Query->result();
        
    }
    
    public function Retailer_collection()
    {
         $Query = $this->db->query(' SELECT tbl_admin.id,tbl_admin.first_name,tbl_admin.last_name, axo_bet.*, ax_claim_ticket.* FROM tbl_admin LEFT JOIN (SELECT retailer_id, SUM(total) AS bet FROM axo_bet  where Batch_id='.$batch_id.' GROUP BY retailer_id) as axo_bet ON tbl_admin.id = axo_bet.retailer_id LEFT JOIN (SELECT retailer_id, SUM(winning_amount) AS wining FROM ax_claim_ticket  where  Batch_id='.$batch_id.' GROUP BY retailer_id) as ax_claim_ticket ON tbl_admin.id = ax_claim_ticket.retailer_id WHERE role != "admin";');
    ...
    public function buy(){
        $sql = $this->db->get('ax_claim_ticket');
    ...
    public function AllUserList()
    {
        ...
        $this->db->where('tbl_admin.role','retailer');
      ...
      public function View_bet_history($user_id)
    {
         
        $Query = $this->db->where('retailer_id', $user_id)
                      ->get('axo_bet');
        return $Query->result();
    }
    
    public function RetailerCoin()
    {
