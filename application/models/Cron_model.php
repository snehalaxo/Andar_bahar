...
class Cron_model extends MY_Model
{
   ...
   return  $last_id;
    ...
    public function UpdateBatch($date,$user_id,$time,$status)
    {
    //   var_dump($date, $time,$user_id,$status);die();
    ...
    //     return true;
         ...
         $query=$this->db->query("update ax_batch SET date='$date',time='$time',status='$status' ,updated_at=date('Y-m-d H:i:s') where id=$user_id");
    }
    
    
      public function andar_card()
    {
        $this->db->select('ax_andar_card.*');
    ...
    return $query->result_array();
     ...
     public function bahar_card()
    {
        $this->db->select('ax_bahar_card.*');
    ...
    return $query->result_array();
       ...
       public function andar_card_status()
    {
        $this->db->select('ax_andar_card_status.*');
        $this->db->from('ax_andar_card_status');
        // $this->db->where('batch_id', $batch_id);
         $this->db->limit(1); 
                 $this->db->order_by('id', 'desc');
    ...
    return $query->result_array();
     ...
     public function bahar_card_status()
    {
        $this->db->select('ax_bahar_card_status.*');
        $this->db->from('ax_bahar_card_status');
                $this->db->order_by('id', 'desc');
