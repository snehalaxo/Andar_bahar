...
class AdminWithdrawalLog_model extends MY_Model
{
 ...
 $id=$this->session->userdata('admin_id'); 
        ...
        return $Query = $this->db->select('tbl_withdrawal_log.*,tbl_admin.first_name as user_name,tbl_admin.whats_no as user_mobile')
            ->from('tbl_withdrawal_log')
            ->join('tbl_admin', 'tbl_admin.id=tbl_withdrawal_log.user_id')
            //->where('tbl_withdrawal_log.isDeleted', FALSE)
            ->where('tbl_withdrawal_log.status', $status)
             ->where('tbl_withdrawal_log.user_id', $id)
            ->get()
            ->result();
    }
