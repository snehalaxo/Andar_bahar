...
class Rummy_model extends MY_Model
{
    ...
    {
        $this->db->select('tbl_rummy_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet');
    ...
    {
        $this->db->select('tbl_rummy_card.*,tbl_users.name');
        $this->db->from('tbl_rummy_card');
    ...
    public function GameLog($game_id, $limit = '',$status = '',$user_id = '')
    {
        $this->db->from('tbl_rummy_log');
        $this->db->where('game_id', $game_id);
        if (!empty($status)) {
    ...
    public function CardValue($joker, $card1, $card2, $card3, $card4='', $card5='', $card6='', $card7='', $card8='', $card9='')
    {
        ...
        else if($card2_color!='JK')
        {
            $set = $card2_num_set;
            $color_group = $card2_color;
        }
        else if($card3_color!='JK')
        {
            $set = $card3_num_set;
            $color_group = $card3_color;
        }
        else if(isset($card4_color) && $card4_color!='JK')
        {
            $set = $card4_num_set;
            $color_group = $card4_color;
        }
        else if(isset($card5_color) && $card5_color!='JK')
        {
        ...
        else if (($card1_num_set == $card2_num_set) && ($card2_num_set == $card3_num_set)) {
            if(empty($card4))
            {
                $set = str_replace(
                    array("J", "Q", "K", "A"),
    ...
    public function GetRamdomGameCard($game_id)
    {
        ...
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_card` WHERE `game_id`='.$game_id.')', NULL, FALSE);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_card_drop` WHERE `game_id`='.$game_id.')', NULL, FALSE);
        $this->db->limit(1);
        $this->db->order_by('RAND()');
        $Query = $this->db->get();
        // echo $this->db->last_query();
